...
-------------------------------------------------------------
-- INITIAL SETUP (tables from assignment)
-------------------------------------------------------------
...
INSERT INTO tickets (event, available, price) VALUES
    ('Concert', 100, 50.00),
    ('Theater', 50, 75.00),
    ('Sports Game', 200, 35.00);
...
-- Scenario A
BEGIN;
UPDATE wallets SET balance = balance + 100 WHERE user_name='Emma';
UPDATE wallets SET balance = balance - 50 WHERE user_name='James';
ROLLBACK;

-- Emma's final balance: 500.00
-- James's final balance: 300.00
-- (ROLLBACK removed all changes)


-- Scenario B
BEGIN;
UPDATE tickets SET available = available - 5 WHERE event='Concert';
COMMIT;     -- Concert available: 95

UPDATE tickets SET available = available - 3 WHERE event='Concert';
ROLLBACK;   -- Undo only this update
...
INSERT INTO wallets (user_name, balance) VALUES ('Sophie', 200.00);

ROLLBACK TO before_delete; -- Undo INSERT, keep DELETE
COMMIT;

-- Does Sophie exist? No
-- Balance? N/A (row deleted)
...
-- User A at T2:
-- SELECT available FROM tickets → 100
-- Answer 2.1: 100
...
-- If SERIALIZABLE:
-- User A sees the original snapshot → 100
-- Answer 2.3: 100

-- Read phenomenon:
-- Answer 2.4: Non-Repeatable Read
...
-- 3.1 After COMMIT data survives crash → Durability
-- Answer: D

-- 3.2 Total money stays same → Consistency
-- Answer: B

-- 3.3 Fail halfway applies nothing → Atomicity
-- Answer: A

-- 3.4 Transactions don't interfere → Isolation
-- Answer: C
